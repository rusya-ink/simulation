...
public class Fish2 extends Entity {
    public Fish2(int y, int x) {
        maxEnergy = 1000;
        energy = 10;
        costOfLife = 5;
        costOfMoving = 50;
            ...
            for (int i = st; i !=fn; i+=d) {
                for (int j = st; j < fn; j+=d) {
                    if (y + i >= 0 && y + i < Cons.HEIGHT && x + j >= 0 && x + j < Cons.WIDTH && (i != 0 || j != 0)) {
                        if (World.ar_block[y + i][x + j].sun > nextSun && (World.ar_block[y + i][x + j].entity instanceof Water || World.ar_block[y + i][x + j].entity instanceof Fish1)) {
    ...
    public void give_birth(int y, int x) {
        ...
        World.ar_block[y][x].entity.energy = energy / 2;
        energy = 0;
    }
}
